...
__global__ void image_domain_adjoint_kernel(float * S, float * Ts, float * Tr, float * data, float * model, float dt, int nt, 
                                            float dx, float dz, int nxx, int nzz, int nb, float aperture, float cmp)
{
    ...
    if ((i >= nb) && (i < nzz-nb) && (j >= nb) && (j < nxx-nb))
    {
        ...
        float z = __int2float_rd(i - nb)*dz;
        float x = __int2float_rd(j - nb)*dx;
        ...
        if (tId < nt)
        {
            ...
            float d2Ts_dz2 = (Ts[(i+1) + j*nzz] - 2.0f*Ts[i + j*nzz] + Ts[(i-1) + j*nzz]) / (dz*dz); 
            float d2Ts_dx2 = (Ts[i + (j+1)*nzz] - 2.0f*Ts[i + j*nzz] + Ts[i + (j-1)*nzz]) / (dx*dx); 
           
            float d2Ts_dxdz = (Ts[(i+1) + (j+1)*nzz] - Ts[(i-1) + (j+1)*nzz] - 
                               Ts[(i+1) + (j-1)*nzz] + Ts[(i-1) + (j-1)*nzz]) / (4.0f*dx*dz);

            float dTr_dz = 0.5f*(Tr[(i+1) + j*nzz] - Tr[(i-1) + j*nzz]) / dz;            
            float dTr_dx = 0.5f*(Tr[i + (j+1)*nzz] - Tr[i + (j-1)*nzz]) / dx;

            float d2Tr_dz2 = (Tr[(i+1) + j*nzz] - 2.0f*Tr[i + j*nzz] + Tr[(i-1) + j*nzz]) / (dz*dz); 
            float d2Tr_dx2 = (Tr[i + (j+1)*nzz] - 2.0f*Tr[i + j*nzz] + Tr[i + (j-1)*nzz]) / (dx*dx); 
           
            float d2Tr_dxdz = (Tr[(i+1) + (j+1)*nzz] - Tr[(i-1) + (j+1)*nzz] - 
                               Tr[(i+1) + (j-1)*nzz] + Tr[(i-1) + (j-1)*nzz]) / (4.0f*dx*dz);
            ...
            float Jterm = sqrtf(detHs * detHr) * S[index] / (norm_Ts * norm_Tr);    
            ...
            float taper = expf(-0.5f*((x - cmp) / (sigma + EPS))*((x - cmp) / (sigma + EPS)));
...
__global__ void angle_domain_adjoint_kernel(float * S, float * Ts, float * Tr, float * data, float * model, float dx, float dz, float dt, 
                                            float da, int nxx, int nzz, int nt, int na, int nb, float aperture, float cmp, int cmpId)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void angle_domain_forward_kernel(float * S, float * Ts, float * Tr, float * data, float * model, float dx, float dz, float dt, 
                                            float da, int nxx, int nzz, int nt, int na, int nb, float aperture, float cmp, int cmpId)
{
    int index = blockIdx.x * blockDim.x + threadIdx.x;
