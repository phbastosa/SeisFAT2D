...
SPS = np.loadtxt("../inputs/geometry/marmousi2_streamer_SPS.txt", delimiter = ",", dtype = np.float32) 
RPS = np.loadtxt("../inputs/geometry/marmousi2_streamer_RPS.txt", delimiter = ",", dtype = np.float32) 
XPS = np.loadtxt("../inputs/geometry/marmousi2_streamer_XPS.txt", delimiter = ",", dtype = np.int32) 
...
for sId in range(ns):
    ...
    data_file = f"../../FWI2D/outputs/data/seismogram_nt10001_nr117_500us_shot_{sId+1}.bin"
    ...
    output_path = f"../inputs/data/marmousi2_shot_{sId+1}.bin"
